...
var set = {
  ...
  echo 'locs: '.json_encode($LOC->getLocales()).',';
    ...
    echo 'timezones: '.json_encode(DateTimeZone::listIdentifiers()).',';
    echo 'currentTimezone: "'.(Settings::get('timezone') ?: 'UTC').'",';
  ...
  setTimezoneList: function () {
    var html = '';
    for (var i = 0; i < set.timezones.length; i++) {
      var tz = set.timezones[i];
      html += '<option value="' + tz + '"';
      if (tz === set.currentTimezone) html += ' selected';
      html += '>' + tz + '</option>';
    }
    $('#timezone').html(html).show();
  },
