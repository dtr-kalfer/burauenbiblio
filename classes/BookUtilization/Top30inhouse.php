...
{
    public function make_top30inhouselist(?string $selectedMonth = null, $limit = 30): array
    {
        try {
            ...
            if (!empty($selectedMonth) && preg_match('/^\d{4}-\d{2}$/', $selectedMonth)) {
                ...
                $dateClause = "ba.activity_time BETWEEN '{$startDate}' AND '{$endDate}'";
            } else {
                // Default fallback: Last 30 days
                $dateClause = "ba.activity_time BETWEEN DATE_SUB(CURDATE(), INTERVAL 1 MONTH) AND NOW()";
            }
