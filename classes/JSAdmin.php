...
class Admin {
  ...
  // construct the initial 'List' screen from database table content
    ...
    addFuncBtns (ident) {
      var html = '';
      html  = '   <input type="button" id="row'+ident+'" class="editBtn" value="'+<?php echo "'".T("Check")."'"; ?>+'" />\n';
  ...
  doDeltBtn (e) {
    ...
    // 🐛 FIXED: iterate full array (not length-1) and handle splice index shift
    // by iterating backwards so removed elements don't affect remaining indices.
    ...
    // 🐛 FIXED: removed fake immediate addHandler call — let the AJAX callback handle the real response
      $.post(this.url, parms, $.proxy(this.addHandler, this), 'json')
      .fail(function(jqXHR, textStatus, errorThrown) {
        console.error('AJAX addNew failed:', textStatus, errorThrown, jqXHR.responseText);
        obib.showError('Server error: ' + textStatus);
      });
      return false;
    };
