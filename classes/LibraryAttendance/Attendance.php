<?php
  ...
  class Attendance extends \ConnectDB { //use backslash (// root namespace if ConnectDB has no namespace)
      ...
      public function countAttendance() {
          return intval($this->selectValue("SELECT COUNT(*) FROM library_attendance"));
      }

      public function getAttendancePage($page = 1, $perPage = 25) {
          $page     = max(1, intval($page));
          $perPage  = max(1, intval($perPage));
          $offset   = ($page - 1) * $perPage;

          $stmt = $this->select(
              "SELECT * FROM library_attendance ORDER BY id DESC LIMIT ? OFFSET ?",
              "ii",
              [$perPage, $offset]
          );
          return $stmt ? $stmt : [];
