 ...
 */
	...
	$tab = 'opac';
  $nav = "free-ebook-gutenberg.php";

	Page::header(array('nav'=>$tab.'/'.$nav, 'title'=>''));

	// index.php — BurauenBiblio + Gutendex eBook Search
...
<section class="container">
    ...
    <?php foreach ($books as $book): ?>
        ...
        $authorStr = htmlspecialchars(implode(', ', $authors));
        ...
        <div class="card">
            <div class="title"><?= $title ?></div>
						<?php 
							$authorStr = ($authorStr == '') ? 'n.a.' : $authorStr;
						?>
            ...
            <div class="formats">
                ...
                if (!empty($formats['application/epub+zip'])) {
                    echo '<a class="format-btn" href="download.php?id=' . $id . '">📘 EPUB</a>';
                    $added = true;
                }
                if (!empty($formats['text/plain; charset=utf-8']) || !empty($formats['text/plain; charset=us-ascii'])) {
                    echo '<a class="format-btn" href="download.php?id=' . $id . '&type=txt">📄 TXT</a>';
                    $added = true;
                }
                if (!empty($formats['application/x-mobipocket-ebook'])) {
                    echo '<a class="format-btn" href="download.php?id=' . $id . '&type=mobi">📱 MOBI</a>';
                    $added = true;
                }
                if (!$added) {
                    echo '<span class="note">No downloadable formats available</span>';
                }
                ?>
            </div>
        </div>
