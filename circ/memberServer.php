...
//      $opts = $settings->getAll();
    ...
    case 'getNewBarCd':
      ...
      break;
    case 'doBarcdSearch':
      // FIX: search by QR code (student/faculty ID number) instead of
      // the internal library card barcode --F.
      // Falls back to the old card-barcode lookup if no QR match, so
      // members not yet encoded with a QR number remain findable.
      $mbrDflt = $members->getMbrByQrcode($_POST['barcdNmbr']);
      if (!is_array($mbrDflt)) {
