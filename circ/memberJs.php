...
var mf = {
  ...
  init: function () {
    ...
    // FIX: wire up search-result pagination buttons (they had no handlers,
    // so Next/Previous did nothing) --F.
    $('.goPrevBtn').on('click',null,mf.goPrevPage);
    $('.goNextBtn').on('click',null,mf.goNextPage);
...
// ----------------------------- improved for pagination function for namesearch result -- F.  Tumulak
  ...
  goNextPage: function () {
    var results = mf.allResults || [];
    var totalPages = Math.max(1, Math.ceil(results.length / (mf.pageSize || 25)));
    if ((mf.currentPage || 1) < totalPages) {
      mf.currentPage++;
      mf.renderPage();
    }
  },
  ...
  doNameSearch: function () {
    ...
    $.ajax({
      ...
      success: function (results) {
        // FIX: normalize server response before paging.
        // The server merges two queries (surname + legal-name), so the
        // same member can appear twice; dedupe by mbrid, keeping the
        // richer row (the one carrying legal-name fields).
        if (!Array.isArray(results)) {
          console.error('doNameSearch: expected array, got:', typeof results);
          results = [];
        }
        let seen = {};
        let merged = [];
        for (let i = 0; i < results.length; i++) {
          let r = results[i];
          if (seen[r.mbrid] === undefined) {
            seen[r.mbrid] = merged.length;
            merged.push(r);
          } else if (r.hasOwnProperty('first_legal_name') || r.hasOwnProperty('last_legal_name')) {
            merged[seen[r.mbrid]] = r;
          }
        }

        mf.allResults = merged; // store results globally
        mf.currentPage = 1;
        mf.pageSize = 25;
        mf.renderPage();
  ...
  renderPage: function () {
    ...
    // FIX: guard against a malformed server response (e.g. an object instead
    // of an array). If results is not a proper array, convert or bail out so
    // we never dump every row into the table at once.
    if (!Array.isArray(results)) {
      console.error('renderPage: expected array, got:', typeof results, results);
      results = [];
      mf.allResults = results;
    }

    let totalPages = Math.max(1, Math.ceil(results.length / pageSize));
    ...
    $('.goNextBtn').prop('disabled', page >= totalPages);
  },
