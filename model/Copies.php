 ...
 *   - array() → [] syntax
 *   - $_SESSION / $_POST null guards
 *   - Fixed: isDuplicateBarcd() dead $errors[] assignment
 *   - Fixed: updateCopy() fetchAll() indexed as single row
 *   - Fixed: getHoldMember() redundant execute() call
 *   - Fixed: getNewBarCode() null guard for empty table
 *   - Fixed: getBibsForCpys() global $opts null guard
 */
...
class Copies extends CoreTable {
  ...
  public function getByBarcode($barcode) {
    ...
    if ($numRows == 0) {
      ...
      Fatal::internalError(T("Duplicate barcode: %barcode%", ['barcode'=>$barcode]));
    }
  }
/*
...
*/
  ...
  public function getBibsForCpys ($barcode_list) {
    global $opts;
    // 🐛 FIXED: $opts may not be set if called outside normal request flow
    $barcdWidth = $opts['barcdWidth'] ?? 13;
    $copies = new Copies;
    # build an array of barcodes
    $barcodes = [];
    foreach (explode("\n", $barcode_list) as $b) {
      if (trim($b) != "") {
        $barcodes[] = str_pad(trim($b), $barcdWidth, '0', STR_PAD_LEFT);
      }
    }
  ...
  public function getCopyInfo ($bibid) {
    ...
    $bcopies = $copies->getMatches(['bibid'=>$bibid],'barcode_nmbr');
    ...
    while ($copy = $bcopies->fetch_assoc()) {
      ...
      if (($_SESSION['multi_site_func'] ?? 0) > 0) {
      ...
      if(($_SESSION['show_checkout_mbr'] ?? 'N') == "Y" && ($status['status_cd'] == OBIB_STATUS_OUT || $status['status_cd'] == OBIB_STATUS_ON_HOLD)){
      ...
      foreach($fieldList as $key => $value){
        $copy['custFields'][] = ['code' => $key, 'data' => $value];
      }
  ...
  public function insertCopy($bibid,$copyid) {
      ...
      $sql = "INSERT `biblio_copy` SET "
            ...
            ."`create_dt` = NOW(),"
            ."`last_change_dt` = NOW(),"
            ."`last_change_userid` = " . ($_SESSION['userid'] ?? 0) . ","
            ."`copy_desc` = $copy_desc";
  ...
  public function updateCopy($bibid,$copyid) {
      ...
      // 🐛 FIXED: fetchAll() returns array of rows, need [0] for first row
      $rows = $rslt->fetchAll();
      $rcd = $rows[0] ?? [];
      $histid = $rcd['histid'] ?? '';
  ...
  protected function validate_el($copy, $insert) {
    $errors = [];
    foreach (['bibid', 'barcode_nmbr'] as $req) {
  ...
  public function isDuplicateBarcd($barcd,$cpyid) {
    ...
    // 🐛 FIXED: removed dead $errors[] assignment — $errors was undefined in this scope
    // and the caller (validate_el) handles its own FieldError creation.
    if (isset($barcd)) {
  ...
  public function getHoldMember($copyid) {
    ...
    // 🐛 FIXED: removed redundant $holds->execute() — select() already executed the query
  ...
  public function lookupBulk_el($barcodes) {
    ...
    return [$copyids, $bibids, $errors, $barcodes];
  }
...
foreach ($rslt as $row) {
    ...
    if ($row['status_cd'] == OBIB_STATUS_IN) {
        // See on which site
        if (($_SESSION['current_site'] ?? '') == $row['siteid'] || !(($_SESSION['multi_site_func'] ?? 0) > 0)) {
  ...
  public function lookupNoCopies($bibids, $del_copyids) {
    ...
    foreach ($bibids as $bibid) {
      $has_copies = false;
      $copies = $this->getMatches(['bibid'=>$bibid]);
  ...
  */
  ...
  public function getCustomFields($copyid, $arrayWanted=false) {
    $flds = [];
    $rslt = $this->custom->getMatches(['copyid'=>$copyid]);
    if ($arrayWanted) {
      //while ($row = $rslt->fetch_assoc()) {
            foreach ($rslt as $row) {
        $flds[] = $row;
  ...
  public function deleteCustomFields($copyid) {
    return $this->custom->deleteMatches(['copyid'=>$copyid]);
  }
  ...
  public function setCustomFields($bibid, $copyid, $customFldsarr) { //Fixed: Warning: Use of undefined constant bibid - assumed 'bibid' --F.T.
    $this->custom->deleteMatches(['copyid'=>$copyid]);
