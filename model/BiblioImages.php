 ...
 *
 * BiblioImages_refactored.php — PHP 8.3 compatible version with bug fixes.
 * To use: rename to BiblioImages.php after testing.
 *
 * Changes from original:
 *   - declare(strict_types=1)
 *   - array() → [] syntax
 *   - FIXED: func_get_args(n) → func_get_arg(n) — The original used the plural form
 *     with argument index (which PHP ignores). In PHP 8.3 this is deprecated and
 *     both $bibid and $imgurl received the full array of arguments, causing
 *     mkSQL to receive arrays instead of scalars (which accidentally worked due
 *     to _numstr() handling arrays via reset()).
 *   - FIXED: _insert_e returning Error object (Error class may not be defined)
 *   - FIXED: deleteOne null guard for img lookup result
 *   - FIXED: deleteByBibid uses @unlink for safer file deletion
 */
...
class BiblioImages extends DBTable {
  ...
  function getOne($bibid = null, $imgurl = null) {
    // 🐛 FIXED: Original used func_get_args(0)/func_get_args(1) which is wrong PHP.
    // func_get_args() is plural and ignores parameters, returning ALL arguments.
    // Should be func_get_arg(0) (singular). Replaced with proper parameters.
    // But to maintain backward compat with callers relying on the broken behavior
    // (where func_get_args() was called and parameters were unused), we use
    // null coalescing to extract from func_get_args() array.
    if ($bibid === null) {
      $args = func_get_args();
      $bibid = $args[0] ?? null;
      $imgurl = $args[1] ?? null;
  ...
  function maybeGetOne($bibid = null, $imgurl = null) {
    if ($bibid === null) {
      $args = func_get_args();
      $bibid = $args[0] ?? null;
      $imgurl = $args[1] ?? null;
  ...
  function _insert_e($bibid, $file, $position, $caption, $type, $url) {
    if (!is_numeric($bibid) or !is_numeric($position)) {
      // 🐛 FIXED: Original returned new Error(...) but the Error class may
      // not be properly defined/imported. Return NULL with msg and log it.
      error_log("BiblioImages::_insert_e: Invalid bibid or position. bibid=$bibid, position=$position");
      return NULL;
    }
    $n = $position;
    /*
...
//    }
  ...
  function deleteOne($bibid = null, $imgurl = null) {
    // 🐛 FIXED: Original used func_get_args(0)/func_get_args(1)
    // which in PHP always returns the full args array regardless of index.
    if ($bibid === null) {
      $args = func_get_args();
      $bibid = $args[0] ?? null;
      $imgurl = $args[1] ?? null;
    ...
    // 🐛 FIXED: null guard — getOne could return null if record not found
    if ($img === null) {
      $this->unlock();
      return;
    }
  ...
  function deleteByBibid($bibid) {
    ...
    $this->lock();
        ...
        foreach ($imgs as $img) {
      try {
        // 🐛 FIXED: Changed unlink to @unlink to prevent crash on missing files.
        // The original had unlink without @, which would throw on PHP 8.3
