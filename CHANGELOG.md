...
- Extended print qr feature to include patrons i.e. faculty or student. This will allow print qr code if present according to their school ID number or faculty number.
- Updated: migrate_old_db.php to include timezone feature and QR code feature. This will allow old db to adopt new features.
- Added on Member: QR code column, design to utilize student ID number (and faculty ID number) into the search query instead of forcing to use autogenerated library ID number. 
- Fixed rendering of result for members to include QR code column.
